...
export class DBInbound {
    ...
    get billingMultiplier(): number {
        return this.enableDoubleBilling ? 2 : 1;
    }

    get billedUsage(): number {
        return (this.up + this.down) * this.billingMultiplier;
    }

    billedClientUsage(stats: ClientStats): number {
        return ((stats.up || 0) + (stats.down || 0)) * this.billingMultiplier;
    }

    clientRemaining(stats: ClientStats): number {
        return Math.max(0, (stats.total || 0) - this.billedClientUsage(stats));
    }

    inboundRemaining(): number {
        return Math.max(0, this.total - this.billedUsage);
    }
