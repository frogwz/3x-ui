...
export class ColorUtils {
  ...
  /** @param multiplier - Billing multiplier, 2 for double billing, 1 (default) for single. */
  static clientUsageColor(clientStats: ClientUsageStats | null | undefined, trafficDiff: number, multiplier: number = 1): string {
    switch (true) {
      case !clientStats || clientStats.total == 0: return COLORS.purple;
      case (clientStats!.up + clientStats!.down) * multiplier < clientStats!.total - trafficDiff: return COLORS.success;
      case (clientStats!.up + clientStats!.down) * multiplier < clientStats!.total: return COLORS.warning;
