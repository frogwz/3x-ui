...
export interface ClientTrafficCellProps {
  ...
  trafficDiff?: number;
  compact?: boolean;
  /** Billing multiplier — 2 when double billing is enabled, 1 (default) otherwise. */
  multiplier?: number;
}
...
export default function ClientTrafficCell({
  ...
  const display = useMemo(
    () => computeTrafficDisplay({ up, down, total, enabled, trafficDiff, multiplier }, isDark),
    [up, down, total, enabled, trafficDiff, multiplier, isDark],
  );
