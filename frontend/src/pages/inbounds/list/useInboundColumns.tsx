    ...
    cols.push(
      ...
      },
      {
        ...
        render: (_, record) => (
            ...
            content={(
              <table cellPadding={2}>
                <tbody>
                  ...
                  {record.total > 0 && record.billedUsage < record.total && (
                    <tr>
                      <td>{t('remained')}</td>
                      <td>{SizeFormatter.sizeFormat(record.inboundRemaining())}</td>
                    </tr>
                  )}
                  <tr>
                    <td>{t('pages.inbounds.billingMode')}</td>
                    <td>
                      <Tag color={record.enableDoubleBilling ? 'orange' : 'blue'}>
                        {record.enableDoubleBilling ? t('pages.inbounds.doubleBilling') : t('pages.inbounds.singleBilling')}
                      </Tag>
                    </td>
                  </tr>
                </tbody>
          ...
          >
            <Tag color={ColorUtils.usageColor(record.billedUsage, trafficDiff, record.total)}>
              {SizeFormatter.sizeFormat(record.billedUsage)} /
              {' '}
