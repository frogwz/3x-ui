...
func (t *Tgbot) getExhausted(chatId int64) {
	...
	for _, inbound := range inbounds {
		if inbound.Enable {
			if (inbound.ExpiryTime > 0 && (inbound.ExpiryTime-now < exDiff)) ||
				(inbound.Total > 0 && (inbound.Total-service.InboundBilledUsage(inbound) < trDiff)) {
				...
				for _, client := range inbound.ClientStats {
					...
					if client.Enable {
						if (client.ExpiryTime > 0 && (client.ExpiryTime-now < exDiff)) ||
							(client.Total > 0 && (client.Total-service.ClientBilledUsage(inbound, &client) < trDiff)) {
	...
	if len(exhaustedInbounds) > 0 {
		...
		for _, inbound := range exhaustedInbounds {
			...
			output += t.I18nBot("tgbot.messages.traffic", "Total=="+common.FormatTraffic(service.InboundBilledUsage(&inbound)), "Upload=="+common.FormatTraffic(inbound.Up), "Download=="+common.FormatTraffic(inbound.Down))
									...
									for _, traffic := range traffics {
										if traffic.Enable {
											if (traffic.ExpiryTime > 0 && (traffic.ExpiryTime-now < exDiff)) ||
												(traffic.Total > 0 && (traffic.Total-service.ClientBilledUsage(inbound, traffic) < trDiff)) {
