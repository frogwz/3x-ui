...
func (s *InboundService) disableInvalidInbounds(tx *gorm.DB) (bool, int64, error) {
	...
	if p != nil {
		...
		err := tx.Table("inbounds").
			Select("inbounds.tag").
			Where("((total > 0 and (up + down) * (CASE WHEN enable_double_billing THEN 2 ELSE 1 END) >= total) or (expiry_time > 0 and expiry_time <= ?)) and enable = ? and node_id IS NULL", now, true).
			Scan(&tags).Error
		if err != nil {
			return false, 0, err
		}
...
// clientBilledUsageExpr scales a client_traffics row's (up+down) by its billing
// multiplier: x2 when any inbound the client belongs to opts into bidirectional
// billing. client_traffics.inbound_id is stale, so membership is resolved through
// the authoritative client_inbounds join.
const clientBilledUsageExpr = `((up + down) * (CASE WHEN (
	EXISTS (
		SELECT 1 FROM client_inbounds ci
		JOIN clients c ON c.id = ci.client_id
		JOIN inbounds i ON i.id = ci.inbound_id
		WHERE c.email = client_traffics.email AND i.enable_double_billing
	)
	OR EXISTS (
		SELECT 1 FROM inbounds i
		WHERE i.id = client_traffics.inbound_id AND i.enable_double_billing
	)
) THEN 2 ELSE 1 END))`

// anyDoubleBilling reports whether at least one inbound opts into bidirectional
// billing. When none do (the common case) the depletion predicate is left
// untouched so the optimized scan keeps its original cost.
func anyDoubleBilling(tx *gorm.DB) bool {
	var probe int64
	err := tx.Model(model.Inbound{}).Where("enable_double_billing = ?", true).Count(&probe).Error
...
// expiry_time placeholder, so callers pass identical args either way. When any
// inbound enables bidirectional billing, the local usage term is scaled so a
// double-billed client depletes at twice the rate.
func depletedCond(tx *gorm.DB) string {
	cond := depletedClientsCondLocal
	var probe int64
	if err := tx.Model(&model.ClientGlobalTraffic{}).Limit(1).Count(&probe).Error; err == nil && probe > 0 {
		cond = depletedClientsCond
	}
	if anyDoubleBilling(tx) {
		cond = strings.Replace(cond, "up + down >= total", clientBilledUsageExpr+" >= total", 1)
	}
	return cond
}
